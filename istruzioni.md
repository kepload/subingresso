...
- **Trigger duplicati rimossi** (30 apr 2026): annunci aveva 3 trigger AFTER INSERT che chiamavano la stessa edge function `notify-alert` — 2 sincroni (`notify-alert` + `notify_alert_update` da webhook UI) + 1 async custom (`notify_alert_trigger` con pg_net). INSERT impiegava 15-20s. Patch in `PATCH_REMOVE_DUPLICATE_TRIGGERS_20260430.sql`. Dopo il fix: INSERT <700ms. **NON ricreare i webhook `notify-alert` o `notify_alert_update` dalla UI Supabase** — basta il trigger custom async.
