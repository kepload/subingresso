...
16. **Serie "Mercati settimanali [città]" e "Mercati ambulanti [regione]"** (9 mag 2026): 2 nuovi filoni editoriali confermati da SC. Top 10 città (Modena/Brescia/Bergamo/Milano/Bologna/Torino/Verona/Padova/Firenze/Napoli) + 5 regioni rimaste (Lombardia/Veneto/Emilia/Toscana/Lazio). **Filone "mercati ambulanti [regione]" APERTO il 16 mag 2026**: pubblicato `mercati-ambulanti-lombardia` (cat `Guide Regionali`, autore `Redazione Subingresso`, 4252 char, cover NULL, `published_at=now()`). Angolo *discovery* (quali mercati / dove rende il banco) distinto da `bandi-posteggi-mercatali-lombardia` (concorso) e `posteggi-mercatali-lombardia-guida-acquisto-vendita` (processo) → no cannibalizzazione. CTA `/annunci?regione=Lombardia` (forma SENZA `.html` → tracciata dal listener `cta_annunci_click`) al picco tensione + close valutatore/annunci/vendi. **Restano**: Veneto/Emilia/Toscana/Lazio (regioni) + serie città. **SC**: richiedere indicizzazione manuale dei 2 nuovi (`quanto-costa-un-posteggio-al-mercato` priorità #1 — query 8 impr/0 clic; `mercati-ambulanti-lombardia` #2) + ri-submit hub Bolkestein dopo rewrite 20.1.
...
20. **Mosse SEO post-pubblicazione Bolkestein** (12 mag 2026, da SC ultime 24h: 27 impr / 1 clic / 8 query, 22 impr Bolkestein = 81%). **Punto critico**: `bolkestein ambulanti ultime notizie 2026` ha **11 impr / 0 clic** (CTR 0%) — siamo in SERP sull'hub ma title/desc non convince o posizione bassa. Mosse in ordine di ROI atteso:
    - **(1+3) Riscrivere title + excerpt del hub Bolkestein** con numero + data + tensione (es. "Bolkestein 2026: cosa cambia davvero per 150.000 ambulanti — aggiornato XX maggio"), bumpare `published_at = now()` (prima maintenance mensile committed), aggiungere paragrafo "Aggiornamento maggio 2026" in cronologia. Un singolo SQL upsert su `blog_posts`. 5-10 min.
